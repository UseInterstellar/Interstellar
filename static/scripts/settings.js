...
const switches2 = document.getElementById('2');
...
const switches3 = document.getElementById('3');

if (window.localStorage.getItem('uvOff') != '') {
  if (window.localStorage.getItem('uvOff') == 'true') {
    switches3.checked = true;
  } else {
    switches3.checked = false;
  }
}

switches3.addEventListener('change', (event) => {
  if (event.currentTarget.checked) {
    window.localStorage.setItem('uvOff', 'true');
  } else {
    window.localStorage.setItem('uvOff', 'false');
  }
});
